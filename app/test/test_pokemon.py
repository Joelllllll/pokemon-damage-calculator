...
from app.test.test_main import PIKACHU
...
class TestPokemonStats:
  def test_init(self):
    result = pokemon.PokemonStats("pikachu", pokemon.EVs(), pokemon.IVs(), 100, "Pound")
  ...
  def test_hp_stat(self):
    result = pokemon.PokemonStats("Pikachu", pokemon.EVs(hp=200), pokemon.IVs(hp=31), 100, "Pound")
    assert result.hp == 261

  def test_attack_stat(self):
    result = pokemon.PokemonStats("Pikachu", pokemon.EVs(attack=200), pokemon.IVs(attack=31), 100, "Pound")
    assert result.attack == 196

  def test_defense_stat(self):
    result = pokemon.PokemonStats("Pikachu", pokemon.EVs(defense=200), pokemon.IVs(defense=31), 100, "Pound")
    assert result.defense == 166

  def test_special_attack_stat(self):
    result = pokemon.PokemonStats("Pikachu", pokemon.EVs(special_attack=200), pokemon.IVs(special_attack=31), 100, "Pound")
    assert result.special_attack == 186

  def test_special_defense_stat(self):
    result = pokemon.PokemonStats("Pikachu", pokemon.EVs(special_defense=200), pokemon.IVs(special_defense=31), 100, "Pound")
    assert result.special_defense == 186

  def test_speed_stat(self):
    result = pokemon.PokemonStats("Pikachu", pokemon.EVs(speed=200), pokemon.IVs(speed=31), 100, "Pound")
    assert result.speed == 266

  def test_max_ev_values(self):
    #max value for Evs are  252
    evs = pokemon.EVs(hp=100, attack=252)
    assert evs.__dict__ == {'attack': 252, 'defense': 0, 'hp': 100, 'special_attack': 0, "special_defense": 0, "speed": 0}
  ...
  def test_max_iv_values(self):
    ...
    assert ivs.__dict__ == {'attack': 31, 'defense': 0, 'hp': 31, 'special_attack': 0, "special_defense": 0, "speed": 0}
